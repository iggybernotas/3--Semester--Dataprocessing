...
for track in gpx.tracks:
    for segment in track.segments:
        ...
        for point in segment.points:
            ...
            sql = f"""
            INSERT INTO public.eyes (
            "xpos1","ypos1","xpos2","ypos2","time","coord") VALUES (
            '50'::integer,'50'::integer,'50'::integer,'50'::integer,'{timestamp}'::time with time zone,  '{{{point.latitude}, {point.longitude}}}'::numeric[])
            """
            print(sql)
            cur.execute(sql)
            timestamp += delta
